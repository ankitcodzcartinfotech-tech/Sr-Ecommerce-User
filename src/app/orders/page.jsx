...
/* ── Status config ─────────────────────────────────────────── */
const STATUS = {
  Pending: { color: "text-[#B8922C]", bg: "bg-[#B8922C]/10", border: "border-[#B8922C]/20", dot: "bg-[#B8922C]", icon: Clock },
  Confirmed: { color: "text-[#2D2A26]", bg: "bg-[#2D2A26]/10", border: "border-[#2D2A26]/20", dot: "bg-[#2D2A26]", icon: CheckCircle },
  Processing: { color: "text-[#7A1A2E]", bg: "bg-[#7A1A2E]/10", border: "border-[#7A1A2E]/20", dot: "bg-[#7A1A2E]", icon: Package },
  Shipped: { color: "text-[#1FA971]", bg: "bg-[#1FA971]/10", border: "border-[#1FA971]/20", dot: "bg-[#1FA971]", icon: Truck },
  Delivered: { color: "text-[#1FA971]", bg: "bg-[#1FA971]/10", border: "border-[#1FA971]/20", dot: "bg-[#1FA971]", icon: CheckCircle },
  Cancelled: { color: "text-[#E53935]", bg: "bg-[#E53935]/10", border: "border-[#E53935]/20", dot: "bg-[#E53935]", icon: XCircle },
};
      ...
      {STEPS.map((step, i) => {
        ...
        return (
          <div key={step} className="flex flex-1 items-center">
            <div className={`flex h-6 w-6 lg:h-7 lg:w-7 shrink-0 items-center justify-center rounded-full border-2 text-[10px] lg:text-xs font-bold transition-all duration-500 ${done ? "border-[#e88436] bg-gradient-to-br from-[#e88436] to-[#d67325] text-white shadow-md shadow-[#e88436]/30" : "border-[#E5E0D8] bg-white text-[#A39C93]"
              } ${active ? "ring-4 ring-[#e88436]/20 scale-125" : ""}`}>
              {active && <span className="absolute inset-0 rounded-full animate-ping bg-[#e88436]/20" />}
            ...
            {i < STEPS.length - 1 && (
              <div className="relative flex-1 mx-1 lg:mx-2 h-0.5 lg:h-1 rounded-full bg-[#E5E0D8] overflow-hidden">
                <motion.div 
                  ...
                  className="absolute left-0 top-0 h-full bg-gradient-to-r from-[#e88436] to-[#d67325]" 
                />
              </div>
            )}
          </div>
      ...
      <div className="relative px-4 sm:px-6 py-8 sm:py-10 md:px-10">
        <div className="mx-auto w-full max-w-4xl lg:max-w-5xl xl:max-w-6xl">
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="flex flex-col gap-1">
            <p className="text-[10px] sm:text-xs font-bold uppercase tracking-[0.2em] text-[#B8922C]">My Account</p>
          ...
          {orders.length > 0 && (
            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="mt-8 flex overflow-x-auto gap-3 sm:gap-4 lg:gap-6 pb-4 sm:pb-0 no-scrollbar [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden sm:grid sm:grid-cols-3">
              {[
                { label: "Total Orders", value: orders.length, icon: ShoppingBag, color: "text-[#2D2A26]", bg: "bg-gradient-to-br from-[#F6F3EC] to-white" },
                { label: "In Progress", value: inProgress, icon: Truck, color: "text-[#7A1A2E]", bg: "bg-gradient-to-br from-[#7A1A2E]/10 to-[#7A1A2E]/5" },
                { label: "Total Spent", value: formatCurrency(totalSpent), icon: TrendingUp, color: "text-[#e88436]", bg: "bg-gradient-to-br from-[#e88436]/10 to-[#e88436]/5" },
              ].map((s) => (
      ...
      <div className="mx-auto w-full max-w-4xl lg:max-w-5xl xl:max-w-6xl px-4 sm:px-6 py-4 sm:py-8 md:px-10">
            ...
            {FILTERS.map(tab => {
              ...
              return (
                <button key={tab.value} onClick={() => setFilter(tab.value)}
                  className={`relative group flex shrink-0 items-center gap-2 rounded-full px-5 py-2.5 text-xs lg:text-sm font-semibold transition-colors duration-300 outline-none focus-visible:ring-2 focus-visible:ring-[#7A1A2E]/50 ${isActive
                      ? "text-white"
                      : "bg-white text-[#736B63] hover:bg-gray-50 border border-black/5 hover:border-black/10 hover:shadow-sm"
                    } cursor-pointer`}>
                  {isActive && (
                    <motion.div layoutId="activeFilter" className="absolute inset-0 bg-gradient-to-r from-[#7A1A2E] to-[#92263d] rounded-full shadow-lg shadow-[#7A1A2E]/30" transition={{ type: "spring", bounce: 0.2, duration: 0.6 }} />
                  )}
        ...
        {filtered.length === 0 ? (
          <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}
            ...
            <div className="mb-5 flex h-20 w-20 items-center justify-center rounded-full bg-[#F6F3EC]">
                      ...
                      <div className="p-5 sm:p-7 relative z-10">
                        ...
                        <div className="flex flex-wrap items-start justify-between gap-3 sm:gap-4">
                          <div className="min-w-0 flex-1">
                            ...
                            <div className="mt-1.5 flex flex-wrap items-center gap-3 sm:gap-4 text-xs font-medium text-[#736B63]">
                              <span className="flex items-center gap-1.5"><Calendar size={13} className="text-[#B8922C]" /> Placed {new Date(order.createdAt).toLocaleDateString("en-IN", { day: "numeric", month: "short", year: "numeric" })}</span>
                        ...
                        <div className="mt-6 space-y-3">
                          {preview.map(item => {
                            ...
                            return (
                              <div key={item._id} className="group flex items-center gap-4 rounded-2xl border border-black/5 bg-gradient-to-br from-[#FDFBF7] to-white p-3 transition-all duration-300 hover:shadow-md hover:border-[#e88436]/30">
                                <div className="relative h-16 w-14 sm:h-20 sm:w-16 shrink-0 overflow-hidden rounded-xl bg-[#F6F3EC] shadow-inner group-hover:scale-105 transition-transform duration-500">
                                ...
                                <div className="min-w-0 flex-1">
                                  <p className="line-clamp-1 text-sm sm:text-base font-bold text-[#2D2A26] group-hover:text-[#e88436] transition-colors">{item.productName}</p>
                                  <p className="mt-1 text-xs sm:text-sm text-[#736B63]">Qty: {item.quantity} <span className="mx-1.5 text-[#e88436]/50">•</span> <span className="font-semibold text-[#2D2A26]">{formatCurrency(item.price)}</span> each</p>
                                </div>
                                <p className="shrink-0 text-sm sm:text-base font-black text-[#e88436]">{formatCurrency(item.subtotal)}</p>
                              </div>
                            );
                          })}
                        ...
                        <div className="mt-6 flex flex-col sm:flex-row items-start sm:items-end justify-between gap-4 border-t border-black/5 pt-5">
                          <div className="w-full sm:w-auto flex justify-between sm:block items-end sm:items-start">
                            ...
                            <p className="total-amount-text text-xl sm:text-2xl font-bold text-[#e88436]">{formatCurrency(order.totalAmount)}</p>
                          </div>
                          {/* Action buttons */}
                          <div className="flex flex-wrap sm:flex-nowrap gap-3 w-full sm:w-auto mt-2 sm:mt-0">
                            ...
                            <Link href={`/orders/orders-details/${order._id}`} className="flex flex-1 sm:flex-none justify-center items-center gap-2 rounded-full bg-[#7A1A2E] px-6 py-2.5 text-xs font-bold text-white transition-all hover:bg-[#5E1222] shadow-md hover:shadow-lg hover:-translate-y-0.5 active:scale-95 cursor-pointer">
